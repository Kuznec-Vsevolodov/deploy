...
class BookingPlacesByUserView(APIView):
    def get(self, request, pk):
        booked_places = Booking.objects.filter(user=pk)
        places = []
        booked_places.__dict__
        for place in booked_places:
            if ParkingPlace.objects.get(id=place.parking_place.pk):
               current_place = ParkingPlace.objects.get(id=place.parking_place.pk)
               serializer = Parking_placeSerializer(current_place, many=False)
...
class GetUserData(APIView):
    def get(self, request):
        current_user = request.user
        return Response({'user': current_user.id, 'name': current_user.username})